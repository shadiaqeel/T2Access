...
using T2Access.Web.SPA.VueJs.Resources;
            ...
            using (var response = await httpService.PostAsync("Login", model))
            {
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (claims.Any(x => x.Type == "http://schemas.microsoft.com/ws/2008/06/identity/claims/role" && x.Value == "Admin"))
                    {
                        ...
                        HttpContext.Session.SetString("Role", string.Join(",", claims.Where(x => x.Type == "http://schemas.microsoft.com/ws/2008/06/identity/claims/role").Select(x => x.Value)));
